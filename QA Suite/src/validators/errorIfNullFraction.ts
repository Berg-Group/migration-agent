...
/**
 * Throws an error if the fraction of NULL values in a column exceeds a threshold
 * 
 * @param table Fully-qualified table name (e.g. "public.people_202505")
 * @param column Column to inspect
 * @param threshold Error threshold (0-1), e.g. 0.5 for 50%
 * @returns true if the NULL fraction is below threshold, false otherwise
 */
export async function errorIfNullFraction(
  table: string,
  column: string,
  threshold: number
): Promise<boolean> {
  ...
  if (result.fraction > threshold) {
    log.error(`${table}.${column}: ${nullPercentage}% NULLs (>${threshold * 100}% threshold) - CRITICAL ERROR`);
    return false;
  }

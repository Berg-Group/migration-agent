{{ config(
    materialized = 'table',
    alias = 'people_notes_fix_invenias',
    tags=["invenias"]
) }}
...
SELECT 
    e.itemid AS id,
        lower(
        substring(md5(e."itemid"::text), 9, 4) || '-' ||
        substring(md5(e."itemid"::text), 1, 8) || '-' ||
        substring(md5(e."itemid"::text), 13, 4) || '-' ||
        substring(md5(e."itemid"::text), 17, 4) || '-' ||
        substring(md5(e."itemid"::text), 21, 12)
    ) AS atlas_id,
    e.body AS text,
...
LEFT JOIN internal_persons ip ON ip.person_id = pe.personid
LEFT JOIN 
    "{{this.schema}}"."users_invenias" u ON u.id = e.creatorid 
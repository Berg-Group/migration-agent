{{ config(
    materialized='table',
    alias='sjt_company_contacts_fix',
    tags=["seven20"]
) }}
...
, base AS (
    SELECT
        ...
        lower(
            substring(md5(c.id::text || '-' || coalesce(a.id::text, 'NULL') || '-' || current_date::text), 1, 8) || '-' ||
            substring(md5(c.id::text || '-' || coalesce(a.id::text, 'NULL') || '-' || current_date::text), 9, 4) || '-' ||
            substring(md5(c.id::text || '-' || coalesce(a.id::text, 'NULL') || '-' || current_date::text), 13, 4) || '-' ||
            substring(md5(c.id::text || '-' || coalesce(a.id::text, 'NULL') || '-' || current_date::text), 17, 4) || '-' ||
            substring(md5(c.id::text || '-' || coalesce(a.id::text, 'NULL') || '-' || current_date::text), 21, 12)
        )    AS atlas_id,

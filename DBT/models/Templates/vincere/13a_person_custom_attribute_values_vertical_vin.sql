--candidate_industry connects to vertical
{{ config(
    materialized='table',
    alias='person_custom_attribute_values_vertical_vin'
) }}
...
candidates AS (
    SELECT
        ...
        s.vertical_id AS option_source_id,
        ...
        {{ atlas_uuid("'vincere_person_attribute_value_vertical_' || '{{ var(\"clientName\") }}' || s.id::text") }} AS atlas_id
    FROM
        {{ var("source_database") }}."candidate_industry" s
    LEFT JOIN
        "{{ this.schema }}"."people_vincere" pv
        ON s.candidate_id = pv.id
    LEFT JOIN
        attribute_options ao
        ON s.vertical_id = ao.option_source_id
    CROSS JOIN
        custom_attributes ca
    WHERE
        s.vertical_id IS NOT NULL
)

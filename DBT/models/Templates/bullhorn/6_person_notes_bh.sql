...
WITH person_comments AS (
    SELECT
        ...
        uc.action AS action,
        'manual' AS type,
        TO_CHAR(uc.dateadded::timestamp(0), 'YYYY-MM-DD"T"HH24:MI:SS') AS created_at,
        TO_CHAR(uc.dateadded::timestamp(0), 'YYYY-MM-DD"T"HH24:MI:SS') AS updated_at
    FROM {{ var('source_database') }}."bh_usercomment" uc
    WHERE uc.comments IS NOT NULL AND uc.comments != ''
        AND LOWER(TRIM(uc.action)) NOT IN ('mail merge', 'email/linkedin message', 'email updated', 'out-bound email')
        AND LOWER(TRIM(uc.action)) IN {{ get_agency_filter('notes') }}
),
base AS (
...
regular_notes AS (
    SELECT
        ...
        b.type
    FROM base b
    INNER JOIN {{ ref('1_people_bh') }} p  ON b.person_id = p.id
    LEFT JOIN {{ ref('0_users_bh') }} um ON b.created_by_id = um.id
),
dupe_people_notes AS (
    ...
    LEFT JOIN {{ ref('0_users_bh') }} um ON um.id = b.created_by_id
)
SELECT 
    id,
    atlas_id,

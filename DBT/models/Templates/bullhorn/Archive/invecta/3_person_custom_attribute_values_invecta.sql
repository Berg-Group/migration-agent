{{ config(
    materialized='table',
    alias='person_custom_attribute_values_invecta',
    tags = ["bullhorn"]
) }}
...
job_title_values AS (
    SELECT DISTINCT
        c.UserID AS person_id,
        TRIM(SPLIT_PART(REPLACE(REPLACE(c.customtext2, ';', ','), ',,', ','), ',', numbers.n)) AS value
    FROM {{ var('source_database') }}.bh_usercontact c
    CROSS JOIN numbers
    WHERE c.customtext2 IS NOT NULL AND TRIM(c.customtext2) != ''
...
specialism_values AS (
    SELECT DISTINCT
        ...
        sl."name" AS value
    FROM {{ var('source_database') }}.bh_usercontact c
    INNER JOIN {{ var('source_database') }}.bh_userskill s ON s.userid = c.userid   
...
desired_location_values AS (
    SELECT DISTINCT
        c.UserID AS person_id,
        c.desiredlocations AS value
    FROM {{ var('source_database') }}.bh_usercontact c
...
business_area_values AS (
    SELECT DISTINCT
        c.UserID AS person_id,
        TRIM(SPLIT_PART(REPLACE(REPLACE(c.division, ';', ','), ',,', ','), ',', numbers.n)) AS value
    FROM {{ var('source_database') }}.bh_client c
    CROSS JOIN numbers
    WHERE c.division IS NOT NULL AND TRIM(c.division) != ''
        AND SPLIT_PART(REPLACE(REPLACE(c.division, ';', ','), ',,', ','), ',', numbers.n) != ''
),
combined_person_values AS (
    SELECT person_id, 'gender' AS attribute_type, value
    FROM gender_values
    UNION ALL
    SELECT person_id, 'job_title' AS attribute_type, value
    FROM job_title_values
    UNION ALL
    SELECT person_id, 'specialism' AS attribute_type, value
    FROM specialism_values
    UNION ALL
    SELECT person_id, 'skillset' AS attribute_type, value
    FROM skillset_values
    UNION ALL
    SELECT person_id, 'desired_location' AS attribute_type, value
    FROM desired_location_values
    UNION ALL
    SELECT person_id, 'business_area' AS attribute_type, value
    FROM business_area_values
)
SELECT DISTINCT

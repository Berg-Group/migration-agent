{{ config(
    materialized='table',
    alias='custom_attribute_options_invecta',
    tags = ["bullhorn"]
) }}
...
WITH internal_attributes AS (
    SELECT
        ...
        alias
    FROM
        {{ ref('1_custom_attributes_invecta') }}
),
numbers AS (
...
specialisms AS (
    SELECT DISTINCT sl."name" AS specialisms
    FROM {{ var('source_database') }}.bh_usercontact c
...
project_specialisms AS (
    SELECT DISTINCT sl."name" AS project_specialisms
...
combined_values AS (
    ...
    FROM job_titles
    UNION ALL
    SELECT 'specialism' AS alias, specialisms AS value
    FROM specialisms
    UNION ALL
    SELECT 'skillset' AS alias, skillset AS value
    FROM skillsets
    UNION ALL
    ...
    UNION ALL
    SELECT 'project_specialism' AS alias, project_specialisms AS value
    FROM project_specialisms
    UNION ALL
    SELECT 'project_skillset' AS alias, customText2 AS value
    FROM project_skillsets
    UNION ALL

...
filtered_jobop AS (
    SELECT
        jo.*
    FROM jobop jo
    WHERE jo.JobPostingID NOT IN (
        SELECT exclude_me FROM excluded_ids
    )
),
projects AS (
    SELECT
        ...
        TO_CHAR(fj.DateAdded::timestamp(0), 'YYYY-MM-DD"T00:00:00Z"') AS created_at,
        TO_CHAR(fj.DateAdded::timestamp(0), 'YYYY-MM-DD"T00:00:00Z"') AS updated_at,
        TO_CHAR(fj.DateClosed::timestamp(0), 'YYYY-MM-DD"T00:00:00Z"') AS closed_at,
...
projects_with_users AS (
    ...
    LEFT JOIN {{ ref('users_ft') }} u ON pwc.owner_id = u.id
),
final_projects AS (
    SELECT
        pwu.*,
        CASE
            WHEN LOWER(pwu.previous_status) IN ('negotiating') THEN 'lead'
            WHEN LOWER(pwu.previous_status) IN (
        ...
        CASE
            WHEN state = 'closed' THEN
                CASE
                    WHEN LOWER(pwu.previous_status) IN ('10 cancelled', '7 on hold / cancelled', 'archive') THEN 'cancelled'
                    WHEN LOWER(pwu.previous_status) IN ('6 placed', 'placed', 'closed-won') THEN 'won'
                    WHEN LOWER(pwu.previous_status) IN ('spec cv sent', 'cancelled') THEN 'lead_lost'
                    ELSE 'worked_lost'
                END
            ELSE NULL
        END AS close_reason,

{{ config(
    materialized = 'table',
    alias = 'project_company_contacts_ft',
    tags=["bullhorn"]
) }}
...
mapped_company_contacts AS (
    ...
    FROM source_data sd
    INNER JOIN {{ ref('5_company_contacts_ft') }} cc ON sd.person_id = cc.person_id
),
mapped_projects AS (
    SELECT
        mcc.project_id,
    ...
    INNER JOIN {{ ref('10_projects_ft') }} p ON mcc.project_id = p.id
)
SELECT
    project_id,
    atlas_project_id,
